        ...
        permits OpaBatchAccessControl
{
    ...
    {
        Optional<String> clientIp = extractClientIp(trinoIdentity);
        return new OpaQueryContext(TrinoIdentity.fromTrinoIdentity(trinoIdentity), pluginContext, Optional.empty(), clientIp);
    }

    OpaQueryContext buildQueryContext(SystemSecurityContext securityContext)
    {
        Optional<String> clientIp = extractClientIp(securityContext.getIdentity());
        return new OpaQueryContext(TrinoIdentity.fromTrinoIdentity(securityContext.getIdentity()), pluginContext, Optional.of(securityContext.getQueryId()), clientIp);
    }

    private Optional<String> extractClientIp(Identity identity)
    {
        Map<String, String> extraCredentials = identity.getExtraCredentials();
        // Try common keys for client IP address
        if (extraCredentials.containsKey("clientIp")) {
            return Optional.of(extraCredentials.get("clientIp"));
        }
        if (extraCredentials.containsKey("remoteAddress")) {
            return Optional.of(extraCredentials.get("remoteAddress"));
        }
        if (extraCredentials.containsKey("remoteUserAddress")) {
            return Optional.of(extraCredentials.get("remoteUserAddress"));
        }
